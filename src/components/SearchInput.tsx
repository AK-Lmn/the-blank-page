...
export default function SearchInput({ value, onChange, onClear }: SearchInputProps) {
    ...
    return <div className="relative">
        ...
        {value && <button type="button" onClick={onClear} aria-label="Clear search" className="absolute right-2 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full text-[#6f8190] transition hover:bg-[#e2e6e9] hover:text-[#28343e] focus:outline-none focus:ring-2 focus:ring-[#72a5c0]"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="m7 7 10 10M17 7 7 17" strokeLinecap="round" /></svg></button>}
    </div>
}
