...
const entry = (id: string, createdAt = new Date().toISOString()): Entry => ({ id, title: id, message: "Message", createdAt, local: true })

describe("local history", () => {
  beforeEach(() => localStorage.clear())

  it("expires entries older than seven days", () => {
    saveToHistory(entry("old", new Date(Date.now() - 8 * 86_400_000).toISOString()))
