...
const mocks = vi.hoisted(() => ({ submitEntry: vi.fn(), saveToHistory: vi.fn() }))
vi.mock("../lib/api", async () => {
  class EntrySubmissionError extends Error { constructor(public status?: number) { super() } }
...
function renderWrite() {
  return render(<MemoryRouter initialEntries={["/write"]}><Routes><Route path="/write" element={<Write />} /><Route path="/entry/:id" element={<p>Entry destination</p>} /></Routes></MemoryRouter>)
}
...
describe("Write", () => {
  ...
  it("navigates using the returned public ID", async () => {
    ...
    mocks.submitEntry.mockResolvedValue({ id: "public-id", title: "Title", message: "Message", createdAt: "2026-01-01" })
    renderWrite()
    ...
    expect(mocks.saveToHistory).toHaveBeenCalledWith(expect.objectContaining({ id: "public-id" }))
  })

  it("shows a retryable failure", async () => {
    const user = userEvent.setup()
    mocks.submitEntry.mockRejectedValueOnce(new Error("network")).mockResolvedValueOnce({ id: "retry", title: "Title", message: "Message", createdAt: "2026-01-01" })
    renderWrite()
    await user.type(screen.getByLabelText("Title"), "Title")
    await user.type(screen.getByLabelText("Message"), "Message")
    await user.click(screen.getByRole("button", { name: "Submit anonymously" }))
  ...
  it("prevents repeated submission while pending", async () => {
    ...
    mocks.submitEntry.mockReturnValue(new Promise((done) => { resolve = done }))
    renderWrite()
    await user.type(screen.getByLabelText("Title"), "Title")
    await user.type(screen.getByLabelText("Message"), "Message")
    ...
    await act(() => { resolve({ id: "one", title: "Title", message: "Message", createdAt: "2026-01-01" }); return Promise.resolve() })
  })

  it("does not submit empty values", async () => {
    renderWrite()
    expect(screen.getByRole("button", { name: "Submit anonymously" })).toBeDisabled()
  })
