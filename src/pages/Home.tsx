    ...
    <section className="mx-auto w-full max-w-[760px] pb-16 pt-14 sm:pb-24 sm:pt-24">
      ...
      <div className="mt-16">
        <div className="mb-5 flex items-baseline justify-between border-b border-[#c5ccd3]/50 dark:border-white/10 pb-3">
          <h2 className="font-display text-2xl tracking-[-0.03em] text-[#28343e] dark:text-[#f0f6fc]">
        ...
        {!loading && error && (
          <div
            role="alert"
            className="mb-3 rounded-[20px] border border-white/60 dark:border-white/10 bg-white/75 dark:bg-[#131b23]/75 backdrop-blur-md px-6 py-5 text-center"
          >
            <p className="text-sm text-[#6f4f70] dark:text-[#d8b4e2]">
              Public entries could not be loaded just now.
            </p>
            <button
              type="button"
              onClick={() => void loadEntries()}
              className="mt-3 text-sm font-medium text-[#2f556a] dark:text-[#72a5c0] underline underline-offset-4 focus:outline-none focus:ring-2 focus:ring-[#72a5c0]"
            >
              Try again
            </button>
          </div>
        ...
        {!loading && !error && entries.length === 0 && (
          <div className="rounded-[20px] border border-dashed border-white/60 dark:border-white/15 bg-white/40 dark:bg-white/5 backdrop-blur-md px-7 py-10 text-center">
            <p className="font-display text-2xl text-[#3c4f5d] dark:text-[#f0f6fc]">
              The page is quiet for now.
            </p>
            <Link
              to="/write"
              className="mt-4 inline-block text-sm font-medium text-[#2f556a] dark:text-[#72a5c0] underline underline-offset-4"
            >
