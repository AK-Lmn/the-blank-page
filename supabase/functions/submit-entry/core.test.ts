...
import { handleSubmitEntry, MAX_BODY_BYTES, type SubmitEntryDependencies } from "./core.ts"

const validRow = { public_id: "public-id", title: "Title", message: "Message", created_at: "2026-01-01T00:00:00Z" }

function deps(overrides: Partial<SubmitEntryDependencies> = {}): SubmitEntryDependencies {
  return {
...
describe("submit-entry handler", () => {
  it("returns 405 for GET", async () => {
    const response = await handleSubmitEntry(new Request("https://function.example", { method: "GET" }), deps())
  ...
  it("returns 413 for an oversized body", async () => {
    expect((await handleSubmitEntry(request("x".repeat(MAX_BODY_BYTES + 1)), deps())).status).toBe(413)
  })
  ...
  it("rejects a populated honeypot without inserting", async () => {
    ...
    const response = await handleSubmitEntry(request({ title: "Title", message: "Message", website: "bot" }), dependencies)
    expect(response.status).toBe(400)
    expect(dependencies.insertEntry).not.toHaveBeenCalled()
  })

  it("returns 429 when rate limited", async () => {
    const response = await handleSubmitEntry(request({ title: "Title", message: "Message" }), deps({
      checkAbuse: vi.fn().mockResolvedValue({ allowed: false, retryAfterSeconds: 60 }),
    }))
  ...
  it("returns 429 for a rapid duplicate", async () => {
    const response = await handleSubmitEntry(request({ title: "Title", message: "Message" }), deps({
      checkAbuse: vi.fn().mockResolvedValue({ allowed: false, retryAfterSeconds: 300 }),
    }))
    expect(response.status).toBe(429)
  })
  ...
  it("inserts only trimmed title and message", async () => {
    const dependencies = deps()
    const response = await handleSubmitEntry(request({ title: "  Title  ", message: "  Message  " }), dependencies)
  ...
  it("returns 500 without exposing insert errors", async () => {
    const response = await handleSubmitEntry(request({ title: "Title", message: "Message" }), deps({
      insertEntry: vi.fn().mockRejectedValue(new Error("secret database error")),
    }))
