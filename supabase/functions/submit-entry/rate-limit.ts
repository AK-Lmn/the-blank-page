...
async function redisPipeline(config: RateLimitConfig, commands: unknown[][]): Promise<RedisResult[]> {
  ...
  if (!Array.isArray(results) || results.some((item) => item.error)) throw new Error("redis-command-failed")
  return results
}
...
export async function checkRateLimit(
  ...
  if (![minute, hour, day].every(Number.isFinite)) throw new Error("invalid-rate-limit-response")
  ...
  if (duplicate[0]?.result !== "OK") return { allowed: false, retryAfterSeconds: 300 }
