...
describe("submission rate limiter", () => {
  ...
  it("enforces the minute limit", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(redisResponse([3, 1, 1, 1, 1, 1])),
    )
    await expect(
      checkRateLimit(request, "title\nmessage", config),
  ...
  it("enforces the hourly and daily safety caps", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock)
    await expect(checkRateLimit(request, "first", config)).resolves.toEqual({
      allowed: false,
      retryAfterSeconds: 3600,
    })
    await expect(checkRateLimit(request, "second", config)).resolves.toEqual({
      allowed: false,
  ...
  it("rejects a duplicate within the cooldown", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock)
    await expect(
      checkRateLimit(request, "same content", config),
